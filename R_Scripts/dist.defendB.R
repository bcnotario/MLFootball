#Distance & Cumulative Distance from Defender

dist.defendB <- function(){
  ...
  for(j in 1:nrow(plays)){
    ...
    if(nrow(track.j)==0 | nrow(track.j[track.j$PositionAbbr=='QB'&!is.na(track.j$event)&track.j$event=='ball_snap',])==0){
      ...
      for(d in 1:nrow(defend.df)){
        ...
        #Cumulative Distance for Defender vs. ALL Receivers
        offens.df <- data.frame(players.j,cdist=0)
        for(m in 1:nrow(offens.df)){
          ...
          offens.df[m,4] <- sum(temp.dist$fdist)
        }
        
        #Cumulative Distance from WR
        defend.df[d,7] <- sum(offens.df[offens.df$PositionAbbr=='WR','cdist'])/(pass.j-snap.j+1)
        
        #Cumulative Distance from TE
        defend.df[d,8] <- sum(offens.df[offens.df$PositionAbbr=='TE','cdist'])/(pass.j-snap.j+1)
        
        #Cumulative Distance from RB
        defend.df[d,9] <- sum(offens.df[offens.df$PositionAbbr=='RB','cdist'])/(pass.j-snap.j+1)
        
        #Cumulative Distance from ALL Recivers
        defend.df[d,10] <- sum(offens.df$cdist)/(pass.j-snap.j+1)
      ...
      for(k in 1:nrow(players.j)){
        ...
        #Yards/frame
        receiv.df[k,6] <- sum(catch.jk$dis)/(pass.j-snap.j+1)
        ...
        receiv.df[k,8] <- min(coverg.df$dist)/(pass.j-snap.j+1)
        
        #Cumulative Distance from ALL Defenders
        receiv.df[k,9] <- sum(coverg.df$cdist)/(pass.j-snap.j+1)
        
        #Cumulative Sideline Distance
        receiv.df[k,10] <- sum(unlist(lapply(53.3-catch.jk$y,catch.jk$y,FUN=min)))/(pass.j-snap.j+1)
      }
